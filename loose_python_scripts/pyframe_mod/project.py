...
#Modified by DG on 10.02.2019 at 19:26
#Create new function that only creates the input files for
#further calculations that I run manually


    def create_inputs(self, system):
#    def create_embedding_potential(self, system):
        ...
        if directories and filenames:
            fragment_sizes, directories, filenames = zip(*sorted(zip(fragment_sizes, directories, filenames),
                                                                 reverse=True))
#Hashed by DG on 10.02.2019 at 19:28 so the calculations are not run
#but instead precomputed *log files are used
...
#                        shutil.rmtree(directory)
    def create_embedding_potential_DG(self, system):
        """Function created by DG on 10.02.2019 at 19:31 in place of create_embedding_potential 
so the new function uses precomputed MpProp files."""
        assert isinstance(system, MolecularSystem)
        if not system.regions:
            # TODO replace with exception
            exit('ERROR: no regions have been added')
        ...
        for region in system.regions.values():
            ...
            for writer in writers:
                ...
                for fragment in fragments.values():
                    ...
                    if os.path.isfile(filename + '.out'):
                        continue
#Hashed by DG on 10.02.2019 at 19:40
#Hashing introduced so no new files are created 
                    if not os.path.isdir(filename):
#                        os.mkdir(filename)
                        pass
                    os.chdir(filename)
